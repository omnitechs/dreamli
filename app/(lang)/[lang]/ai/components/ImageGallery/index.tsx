                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    {gen.images.map((img: any) => (
                        <figure key={img.id}
                            ...
                            <img src={img.url} alt={t('alt', {name: img.meta?.name ?? ''})} className="w-full h-40 object-cover" />
                            ...
                            <div className="absolute inset-0 flex items-start justify-between p-2">
                                <button onClick={() => dispatch(toggleSelect(img.id))}
                                    ...
                                    {selectedSet.has(img.id) ? t('selected') : t('select')}
                                </button>
                                <button onClick={() => dispatch(removeImage(img.id))}
                                        className="px-2 py-1 rounded-md text-xs bg-white/90 hover:bg-white shadow" title={t('remove')}>
                                    {t('remove')}
                                </button>
                            </div>
                            {img.key && (
