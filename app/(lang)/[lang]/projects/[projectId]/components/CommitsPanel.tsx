// app/(lang)/[lang]/projects/[projectId]/components/CommitsPanel.tsx
'use client';

import { useState } from 'react';
import type { CommitJSON, ProjectState } from '../../types';
import { checkoutCommit } from '../actions';
import { Clock, Star, GitBranch } from 'lucide-react';

interface CommitsPanelProps {
    commits: CommitJSON[];
    headId: string;                          // the server "head" (for badge/fallback)
    selectedId?: string;                     // ← NEW: current UI selection (for highlight)
    projectId: string;
    onSelectCommit?: (commitId: string) => void; // ← NEW: local select (no server call)
    onStateUpdate?: (state: ProjectState) => void; // kept for server mutations
}
...
export function CommitsPanel({
    ...
    // use selectedId if provided; otherwise fall back to headId for highlighting
    const activeId = selectedId ?? headId;

    const handleSelectCommit = async (commitId: string) => {
        if (commitId === activeId) return;

        // If the parent provided a local selection handler, use it (NO DB roundtrip).
        if (onSelectCommit) {
            onSelectCommit(commitId);
            return;
        }

        // Fallback: keep your existing server-side checkout behavior.
    ...
    const groupCommitsByDate = (list: CommitJSON[]) => {
            ...
            <div className="flex-1 overflow-auto">
                {Object.entries(commitGroups).map(([dateGroup, dateCommits]) => (
                    ...
                    `}
                          ...
                          ${commit.isVersion ? 'bg-purple-100 text-purple-600' : 'bg-gray-100 text-gray-600'}
                        `}
                                            >
                                                {commit.isVersion ? (
