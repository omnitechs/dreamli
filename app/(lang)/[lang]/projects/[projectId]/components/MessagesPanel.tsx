...
interface MessagesPanelProps {
    messages: Message[];
    projectId: string;
    headId: string;           // keep your existing type
    models?: any[];
    onStateUpdate: (state: ProjectState) => void;
}

export function MessagesPanel({ messages, projectId, headId, models = [], onStateUpdate }: MessagesPanelProps) {
    const [messageText, setMessageText] = useState('');
    const [sending, setSending] = useState(false);
    const [expandedActions, setExpandedActions] = useState<Set<string>>(new Set());

    // 👇 use a scroll container ref instead of "scrollIntoView"
    const scrollRef = useRef<HTMLDivElement>(null);
    const prevScrollHeightRef = useRef<number>(0);
    const userAtBottomRef = useRef<boolean>(true);

    // Helper: are we close to the bottom?
    const nearBottom = (el: HTMLElement) =>
        el.scrollHeight - el.scrollTop - el.clientHeight < 120;

    // Track user's scroll position (so we don't force-scroll)
    const handleScroll = () => {
    ...
    // Preserve scroll position across renders, and only autoscroll when user is near bottom.
    useLayoutEffect(() => {
        const el = scrollRef.current;
        if (!el) return;
        ...
        // If user is NOT at bottom, adjust scrollTop to keep viewport stable
        if (!userAtBottomRef.current && prevHeight > 0) {
            const delta = nextHeight - prevHeight;
            if (delta !== 0) el.scrollTop += delta;
        }

        // If user IS at bottom (or very close), keep them pinned to bottom
        if (userAtBottomRef.current) {
            el.scrollTop = el.scrollHeight;
        }

        prevScrollHeightRef.current = nextHeight;
    }, [messages, models, headId]); // re-run when messages/models/head change
    ...
    const getRoleIcon = (role: string) =>
        role === 'user' ? <User className="w-4 h-4" /> :
            role === 'assistant' ? <Bot className="w-4 h-4" /> :
                role === 'system' ? <Settings className="w-4 h-4" /> : <div className="w-4 h-4 rounded-full bg-gray-400" />;

    const getRoleColor = (role: string) =>
        role === 'user' ? 'text-blue-600 bg-blue-50' :
            role === 'assistant' ? 'text-green-600 bg-green-50' :
                role === 'system' ? 'text-purple-600 bg-purple-50' : 'text-gray-600 bg-gray-50';
                ...
                {Array.isArray(models) && models.length > 0 && (
                    <div className="mb-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                            {models.map((m: any) => {
                                const url = m?.modelUrls?.glb || m?.modelUrls?.fbx || m?.modelUrls?.obj || m?.modelUrls?.usdz;
