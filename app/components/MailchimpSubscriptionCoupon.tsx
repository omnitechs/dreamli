...
export default function MailchimpSubscriptionCoupon() {
    ...
    useEffect(() => {
        ...
        const checkShowPopup = () => {
            ...
            if (hasSubscribed || isRegistered) return;

            if (dismissedAt) {
                const dismissTime = new Date(dismissedAt);
                const now = new Date();
                const tenMinutes = 10 * 60 * 1000;
                if (now.getTime() - dismissTime.getTime() < tenMinutes) return;
            }
                ...
                <div className="p-8">
                    <div className="text-center mb-6">
                        <div className="inline-block bg-gradient-to-r from-yellow-400 to-orange-400 rounded-full px-4 py-2 mb-4">
                            <span className="text-sm font-bold text-gray-900">{t('badge')}</span>
                        </div>
                    ...
                    {!isSubmitted ? (
                        ...
                        <div className="text-center py-8">
                            ...
                            <h4 className="text-xl font-bold text-gray-900 mb-2">{t('success.title')}</h4>
                            <p className="text-gray-700 text-sm">{t('success.body')}</p>
                        </div>
                    )}
                </div>
            </div>
