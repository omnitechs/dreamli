...
export default function UploadForm({ onClose, isNL = false, type, pricingField }: UploadFormProps) {
  ...
  const texts = {
    en: {
      ...
      uploadText: 'Click to upload an image or video',
      supportedFormats: 'Images: JPG, PNG, GIF | Videos: MP4, MOV, AVI (Max: 50MB)',
      ...
      guaranteeTitle: 'Your Satisfaction Guaranteed',
      guaranteeText: "We wait for your confirmation before printing. If you're not happy after 3 revisions, we'll give you your money back - no questions asked.",
      orderButton: 'Order Now',
      processingButton: 'Processing Order...',
      redirectingButton: 'Redirecting to Checkout...',
      successTitle: 'Request Sent!',
      successMessage: 'Thank you for your order! We will contact you within 24 hours with a preview and pricing for confirmation.',
      closeButton: 'Close',
    ...
    nl: {
      ...
      uploadLabel: 'Upload Afbeelding of Video *',
      uploadText: 'Klik om een afbeelding of video te uploaden',
      supportedFormats: 'Afbeeldingen: JPG, PNG, GIF | Videos: MP4, MOV, AVI (Max: 50MB)',
      ...
      guaranteeTitle: 'Uw Tevredenheid Gegarandeerd',
      guaranteeText: 'We wachten op uw bevestiging voordat we printen. Als u niet tevreden bent na 3 revisies, geven we uw geld terug - geen vragen gesteld.',
      orderButton: 'Nu Bestellen',
      processingButton: 'Bestelling Verwerken...',
      redirectingButton: 'Doorverwijzen naar Checkout...',
      successTitle: 'Aanvraag Verzonden!',
      successMessage: 'Dank je wel voor je bestelling! We nemen binnen 24 uur contact met je op met een voorvertoning en prijzen ter bevestiging.',
      closeButton: 'Sluiten',
      requiredError: 'Vul alle verplichte velden in.',
      fileSizeError: 'Bestandsgrootte moet minder dan 50MB zijn.',
      serverError: 'Er is iets misgegaan. Probeer het later opnieuw.'
    }
  };

  // Force Dutch language detection - check URL or explicit prop
  const isDutch = isNL || (typeof window !== 'undefined' && window.location.pathname.includes('/nl'));
  const t = texts[isDutch ? 'nl' : 'en'];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      submitData.append('language', isDutch ? 'nl' : 'en');
      ...
      if (response.ok) {
        ...
        if (result.sessionUrl && result.id) {
          console.log('Stripe checkout session received:', result);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          ...
          {submitStatus !== 'success' ? (
            <form id="dreamli-upload" onSubmit={handleSubmit} className="space-y-5">
              <div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  placeholder={isDutch ? "Voer kortingscode in" : "Enter coupon code"}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#B9E4C9] focus:border-[#B9E4C9] outline-none transition-colors text-sm"
                />
              </div>
