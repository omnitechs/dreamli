...
import BlogHeroDE from './BlogHeroDE';
import CategoryHighlightsDE from './CategoryHighlightsDE';
import FeaturedArticlesDE from './FeaturedArticlesDE';
import LatestPostsDE from './LatestPostsDE';
import TrustSectionDE from './TrustSectionDE';
import NewsletterSignupDE from './NewsletterSignupDE';
import BlogClosingCTADE from './BlogClosingCTADE';
import FooterDE from '../components/FooterDE';
import BlogPageClientDE from './BlogPageClientDE';
...
async function getFeaturedPosts() {
  try {
    const response = await fetch(
      'https://blog.dreamli.nl/wp-json/wp/v2/posts?sticky=true&_embed&lang=de',
      {
        next: { revalidate: 600 },
      }
    );
    ...
    const postsWithCategories = await Promise.all(
      postsWithImages.map(async (post) => {
        ...
        return {
          ...post,
          categoryNames: categoryNames.length > 0 ? categoryNames : ['Allgemein']
        };
      })
    );
...
export default async function BlogPageDE() {
  ...
  return (
    <div className="min-h-screen bg-white">
      <HeaderDE />
      <BlogHeroDE />
      <CategoryHighlightsDE />
      
      {/* === FEATURED_POSTS_SLOT (leave this comment) === */}
      <FeaturedArticlesDE featuredPosts={featuredPosts} />
...
export const metadata = {
  title: 'Dreamli Blog - Kreatives Spielen & Kindliche Entwicklung Einblicke',
  description: 'Entdecken Sie Experteneinblicke über kreatives Spielen, kindliche Entwicklung und Bildungsaktivitäten. Lesen Sie Erfolgsgeschichten und Tipps, um die Fantasie bei Kindern zu fördern.',
  keywords: 'kreatives Spielen, kindliche Entwicklung, Bildungsaktivitäten, Erziehungstipps, Fantasie, MINT-Lernen, Kreativität',
  openGraph: {
    title: 'Dreamli Blog - Kreatives Spielen & Kindliche Entwicklung Einblicke',
    description: 'Experteneinblicke über kreatives Spielen, kindliche Entwicklung und Bildungsaktivitäten, um die Fantasie bei Kindern zu fördern.',
    images: [{
