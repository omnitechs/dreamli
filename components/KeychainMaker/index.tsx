...
type Props = {
    scadPath?: string; // default '/test.scad'
    className?: string;
};
...
export default function KeychainBuilder({ scadPath = '/test.scad', className = '' }: Props) {
    ...
    const { switchTo } = useKeychainThree({
        containerRef,
        defines,
        scadPath,
        freeView,
        ...
        <section className={`max-w-4xl mx-auto p-6 space-y-6 ${className}`}>
            ...
            <div className="grid sm:grid-cols-3 gap-4 items-end">
                ...
                <div className="flex gap-6 items-center">
                    ...
                    <label className="flex items-center gap-2">
                        <input type="checkbox" checked={defines.mostrar_anilla} onChange={(e) => (defines.mostrar_anilla = e.target.checked)} />
            ...
            <div className="grid sm:grid-cols-3 gap-4">
                <label className="flex flex-col gap-1">
                    ...
                    <select
                        ...
                        onChange={(e) =>
                            setColors((c) => (dosColores ? { ...c, baseName: e.target.value } : { ...c, unicoName: e.target.value }))
                        }
                    >
                        {COLOR_OPTIONS.map((c) => (
                ...
                {dosColores && (
                    <label className="flex flex-col gap-1">
                        ...
                        <select
                            className="border rounded px-3 py-2"
                            value={colors.textName}
                            onChange={(e) => setColors((c) => ({ ...c, textName: e.target.value }))}
                        >
                            {COLOR_OPTIONS.map((c) => (
                                <option key={c}>{c}</option>
                            ))}
            ...
            {/* Canvas (pointer events: locked on mobile until Free Rotate) */}
            <div
                ref={containerRef}
