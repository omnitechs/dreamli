...
// @ts-ignore
type MsgIn = { scadPath: string; defines: Record<string, string | number | boolean>; outName?: string; };
...
async function importFromPublic(path: string) { return await import(/* webpackIgnore: true */ path); }

async function getInstance(log: (...a:any[])=>void, logErr:(...a:any[])=>void) {
    if (!instancePromise) {
        instancePromise = (async () => {
            ...
            return await create({ noInitialRun: true, locateFile: (p: string) => `${BASE}/${p}`, print: (...a:any[]) => log(...a), printErr: (...a:any[]) => logErr(...a) });
        })();
    }
    ...
    if (!fontsLoaded) {
        try {
            ...
            if (typeof fontsMod?.addFonts === "function") { await fontsMod.addFonts(inst); log("[fonts] loaded"); }
            else { log("[fonts] helper not present"); }
        } catch (e:any) { logErr("[fonts] failed to import:", String(e)); }
        fontsLoaded = true;
    }
    return inst;
}
...
ctx.onmessage = async (e: MessageEvent<MsgIn>) => {
    ...
    try {
        ...
        try { inst.callMain(args); log("[compile] done"); }
        catch (err:any) {
            const name = err?.name || "";
            const status = (err && typeof err.status === "number") ? err.status : undefined;
            if (name === "ExitStatus" || name === "Exit") logErr(`[compile] exited with status`, String(status ?? "unknown"));
            else logErr("[compile] crash:", String(err?.message || err));
        }

        let stl: Uint8Array | null = null;
        try { stl = inst.FS.readFile(OUT); log("[fs] read", OUT, `(${stl.byteLength} bytes)`); }
        catch (re) { logErr("[fs] read failed:", String(re)); }

        if (!stl || !stl.byteLength) { ctx.postMessage({ ok: false, error: "No STL produced.", logs } as MsgOutErr); return; }
